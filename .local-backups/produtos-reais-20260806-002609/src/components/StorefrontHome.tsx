...
const categoryImages: Record<string, string> = {
  Fidget: "/home-images/categoria-fidget.png",
  Sensorial: "/home-images/categoria-sensorial.png",
  Motor: "/home-images/categoria-motor.png",
  Comunicação: "/home-images/categoria-comunicacao.png",
  Rotina: "/home-images/categoria-rotina.png",
};
...
const collectionTiles = [
  {
    label: "Fidget",
    category: "Fidget",
    image: "/home-images/categoria-fidget.png",
  },
  {
    label: "Sensorial",
    category: "Sensorial",
    image: "/home-images/categoria-sensorial.png",
  },
  {
    label: "Motor",
    category: "Motor",
    image: "/home-images/categoria-motor.png",
  },
  {
    label: "Comunicação",
    category: "Comunicação",
    image: "/home-images/categoria-comunicacao.png",
  },
] as const;

const familyGallery = [
  {
    image: "/home-images/galeria-nova-sensorial.png",
    alt: "Mãe e criança explorando um brinquedo sensorial juntas",
  },
  {
    image: "/home-images/galeria-nova-rotina.png",
    alt: "Pai e criança organizando uma rotina visual de forma lúdica",
  },
  {
    image: "/home-images/galeria-nova-motor.png",
    alt: "Pai e criança brincando com peças de encaixe",
  },
  {
    image: "/home-images/galeria-nova-comunicacao.png",
    alt: "Mãe e criança participando de uma brincadeira com apoio visual",
  },
] as const;
...
export default function StorefrontHome() {
  ...
  const featuredProducts = products.filter((product) => Boolean(product.image)).slice(0, 5);
    ...
    <div className="overflow-hidden bg-background">
      ...
      <section className="border-y border-border/45 bg-background-alt py-18 sm:py-22">
        <div className="mx-auto max-w-7xl px-5 text-center">
          ...
          <div className="mx-auto mt-12 grid max-w-4xl grid-cols-2 justify-items-center gap-x-8 gap-y-10 sm:grid-cols-4">
            {categoryList.slice(0, 4).map((category, index) => {
              ...
              return (
                <Link
                  ...
                  className="category-play group w-full max-w-40 text-center"
                >
                  <div className="storybook-shadow relative mx-auto flex aspect-square w-32 items-center justify-center overflow-hidden rounded-full border-4 border-white bg-background transition duration-300 group-hover:-translate-y-2 group-hover:border-secondary-light/50">
                    ...
                    {categoryImages[category] ? (
                      <img
                        ...
                        alt={`Brincadeira relacionada à categoria ${category}`}
        ...
        <div className="grid items-center gap-10 lg:grid-cols-[1.05fr_0.95fr]">
          <div className="relative min-h-[570px] overflow-hidden rounded-[2rem] storybook-shadow">
            <img
              ...
              alt="Adulto e criança explorando brinquedos juntos com atenção ao interesse da criança"
        ...
        <div className="mt-12 grid min-h-[660px] gap-4 lg:grid-cols-12 lg:grid-rows-2">
          {collectionTiles.map((tile, index) => {
            ...
            return (
              <Link
                key={tile.label}
                href={`/colecoes?categoria=${encodeURIComponent(tile.category)}`}
                className={`group relative min-h-72 overflow-hidden rounded-[1.7rem] ${spans[index]}`}
                aria-label={`Explorar a coleção ${tile.label}`}
              >
                <img
                  src={tile.image}
                  alt={`Brincadeira relacionada à coleção ${tile.label}`}
                  className="absolute inset-0 h-full w-full object-cover transition duration-700 group-hover:scale-105"
                  loading="lazy"
                />
                <div className="absolute inset-0 bg-gradient-to-t from-primary/88 via-primary/12 to-transparent" />
                <div className="absolute bottom-0 left-0 p-7 text-white">
                  <p className="text-[10px] font-black uppercase tracking-[0.18em] text-secondary-light">Coleção</p>
                  <h3 className="mt-2 max-w-md font-display text-4xl leading-none">{tile.label}</h3>
                  <p className="mt-3 text-sm font-bold text-white/75">Explorar produtos →</p>
                </div>
              </Link>
            );
          })}
          ...
          <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
            {familyGallery.map((item) => (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="group relative aspect-square overflow-hidden rounded-[1.5rem] transition duration-500 hover:-translate-y-2"
                aria-label="Ver conteúdos da BrinqueTEAndo no Instagram"
              >
                <img
                  src={item.image}
                  alt={item.alt}
                  ...
                  loading="lazy"
                />
                <span className="absolute inset-0 bg-primary/5 transition group-hover:bg-transparent" />
              </a>
            ))}
          </div>
