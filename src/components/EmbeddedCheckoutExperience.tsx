...
type CheckoutSummary = {
  ...
  discountSource: "progressive" | "coupon" | "none";
  couponCode: string;
...
export default function EmbeddedCheckoutExperience({
  cep,
  shippingId,
  couponCode,
}: Props) {
    ...
    async function mountCheckout() {
      ...
      try {
        ...
        const checkout = await stripe.createEmbeddedCheckoutPage({
          fetchClientSecret: async () => {
            const response = await fetch(
              withBasePath("/api/checkout/embedded"),
              {
                method: "POST",
        ...
        if (!containerRef.current) {
          throw new Error("Área de pagamento indisponível.");
        }
  ...
  }, [cartKey, cep, shippingId, couponCode]);

  if (!items.length) {
    return (
      <div className="mx-auto max-w-2xl px-5 py-20 text-center">
        <div className="mx-auto flex h-24 w-24 items-center justify-center rounded-full bg-background-alt text-5xl">
          🛒
        </div>
        ...
        <Link
          href="/colecoes"
          className="button-shimmer mt-8 inline-flex rounded-full bg-secondary px-8 py-4 font-black text-white"
        >
          Escolher produtos
        </Link>
      </div>
    ...
    return (
      <div className="mx-auto max-w-2xl px-5 py-20 text-center">
        ...
        <p className="mt-4 text-text-light">
          Escolha a entrega no carrinho para que o valor correto seja incluído
          no pagamento.
        </p>
        <Link
          href="/carrinho"
          className="mt-8 inline-flex rounded-full bg-secondary px-8 py-4 font-black text-white"
        >
          Voltar ao carrinho
        </Link>
      </div>
  ...
  const displaySummary: CheckoutSummary = summary || {
    subtotal,
    discountPercent: localDiscount.tier.percent,
    discountAmount: localDiscount.amount,
    discountSource: localDiscount.source,
    couponCode: localDiscount.coupon.valid
      ? localDiscount.coupon.code
      : "",
    couponApplied: localDiscount.coupon.applied,
    shippingAmount: 0,
  ...
  const discountLabel =
    displaySummary.discountSource === "coupon"
      ? `Cupom ${displaySummary.couponCode} (${displaySummary.discountPercent}%)`
      : `Desconto progressivo (${displaySummary.discountPercent}%)`;
        ...
        <div className="mb-8 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
          <div>
            ...
            <h1 className="mt-2 font-display text-5xl text-primary sm:text-6xl">
              Finalize sem sair da loja
            </h1>
            <p className="mt-4 max-w-2xl leading-7 text-text-light">
              Revise seus dados, o endereço e o cartão no formulário seguro da
              Stripe.
            </p>
          </div>
          <Link
            href="/carrinho"
            className="nav-link text-xs font-black uppercase tracking-[0.14em] text-secondary"
          >
            ← Voltar ao carrinho
          </Link>
        </div>
          ...
          <aside className="rounded-[2rem] border border-border/50 bg-white p-6 shadow-[0_22px_55px_rgba(9,38,71,0.09)] lg:sticky lg:top-40">
            ...
            <div className="mt-5 space-y-3 text-sm">
              <div className="flex justify-between gap-3 text-text-light">
                <span>Subtotal</span>
                <strong className="text-primary">
                  {formatMoney(displaySummary.subtotal)}
                </strong>
              </div>
              {displaySummary.discountPercent > 0 ? (
                <div className="flex justify-between gap-3 text-secondary">
                  ...
                  <strong>
                    − {formatMoney(displaySummary.discountAmount)}
                  </strong>
                </div>
              ) : null}
              {displaySummary.couponCode &&
              !displaySummary.couponApplied &&
              displaySummary.discountSource === "progressive" ? (
                <p className="rounded-xl bg-background-alt p-3 text-xs leading-5 text-text-light">
                  O cupom {displaySummary.couponCode} é válido. Mantivemos o
                  desconto progressivo, que é maior.
                </p>
              ) : null}
              <div className="flex justify-between gap-3 text-text-light">
                <span>{displaySummary.shippingLabel}</span>
                <strong className="text-primary">
                  {displaySummary.shippingAmount === 0
                    ? "Grátis"
                    : formatMoney(displaySummary.shippingAmount)}
                </strong>
              </div>
            </div>
            ...
            <div className="mt-5 rounded-2xl bg-background-alt p-4 text-xs leading-5 text-text-light">
              ...
              <p className="mt-2">
                💳 Você poderá autorizar o salvamento seguro do cartão para uma
                compra futura mais rápida.
              </p>
            </div>
          </aside>
          ...
          <section className="min-h-[680px] rounded-[2rem] border border-border/50 bg-white p-3 shadow-[0_24px_70px_rgba(9,38,71,0.1)] sm:p-6">
            {loading ? (
              <div className="flex min-h-[560px] flex-col items-center justify-center text-center">
                ...
                <p className="mt-5 font-bold text-text-light">
            ...
            {error ? (
              <div
                className="m-4 rounded-3xl bg-secondary/10 p-6 text-center"
                role="alert"
              >
                <p className="font-black text-secondary">{error}</p>
                <Link
                  href="/carrinho"
                  className="mt-5 inline-flex rounded-full bg-primary px-6 py-3 text-sm font-black text-white"
                >
                  Revisar o carrinho
                </Link>
              </div>
