  ...
  try {
    ...
    if (!createContactError) {
      ...
      if (!updateContactError) {
        ...
        console.error("Falha ao salvar contato da newsletter:", {
          createContactError,
          updateContactError,
        });
      }
    ...
    if (!contactSaved) {
      return NextResponse.json(
        {
          error:
            "Não conseguimos salvar sua inscrição agora. Aguarde alguns instantes e tente novamente.",
        },
        { status: 502 },
      );
    }
    ...
    try {
      ...
      if (confirmationError) {
        console.error("Falha no e-mail de confirmação da newsletter:", confirmationError);
      } else {
        confirmationEmailSent = true;
      }
    } catch (confirmationError) {
      console.error("Exceção no e-mail de confirmação da newsletter:", confirmationError);
    }
    ...
    try {
      ...
      if (ownerNotificationError) {
        console.error(
          "Falha no aviso de nova inscrição para a loja:",
          ownerNotificationError,
        );
      }
    } catch (ownerNotificationError) {
      console.error(
        "Exceção no aviso de nova inscrição para a loja:",
        ownerNotificationError,
      );
    }
    ...
    return NextResponse.json({
      ...
      message: confirmationEmailSent
        ...
        : "Sua inscrição foi confirmada e seu benefício está liberado. Anote o cupom exibido nesta tela.",
    });
  } catch (error) {
