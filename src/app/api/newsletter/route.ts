...
export async function POST(req: Request) {
  try {
    ...
    if (resendApiKey) {
      ...
      console.error("RESEND_API_KEY ausente na inscrição da newsletter.");
    }

    /*
     * O cupom é uma oferta do site e não pode ficar bloqueado por uma falha
     * temporária do provedor de e-mail ou por uma API key restrita somente a
     * envio. O contato é tentado no Resend e a loja recebe uma notificação
     * sempre que o envio estiver disponível.
     */
    return NextResponse.json({
      ok: true,
