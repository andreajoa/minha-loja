...
export async function POST(req: Request) {
  try {
    ...
    if (contactError) {
      ...
      if (updateError) {
        console.error("Falha ao registrar contato na newsletter:", updateError);
      }
    }
    ...
    if (confirmationError) {
      return NextResponse.json(
        { error: "Não foi possível confirmar sua inscrição agora." },
        { status: 502 },
      );
    }
    ...
    await resend.emails.send({
      ...
      html: `
        <div style="font-family:Arial,sans-serif;color:#24364A;padding:24px">
          ...
          <p><strong>Origem:</strong> ${source === "popup" ? "Pop-up do cupom de 5%" : "Formulário do rodapé"}</p>
        </div>
      `,
    });
